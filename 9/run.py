...
class Measurements:
    ...
    def second_assignment(self) -> int:
        sizes = []
    ...
    def _find_basin_points(self, c: tuple) -> List[tuple]:
        ...
        if c[0] - 1 >= 0:
            if self._compare_points(current_value, self.points[c[0] - 1][c[1]]):  # Check N
                points += self._find_basin_points((c[0] - 1, c[1]))
        
        if c[0] + 1 < len(self.points):
            if self._compare_points(current_value, self.points[c[0] + 1][c[1]]):  # Check S
                points += self._find_basin_points((c[0] + 1, c[1]))

        if c[1] - 1 >= 0:
            if self._compare_points(current_value, self.points[c[0]][c[1] - 1]):  # Check W
                points += self._find_basin_points((c[0], c[1] - 1))

        if c[1] + 1 < len(self.points[0]):
            if self._compare_points(current_value, self.points[c[0]][c[1] + 1]):  # Check E
                points += self._find_basin_points((c[0], c[1] + 1))

        return points


def first_assignment(m: Measurements) -> int:
