...
def first_assignment(navigations: List[List[str]]) -> int:
    wrong_chars = find_wrong_chars(navigations)
    return sum([POINTS_INCORRECT[c]for c in wrong_chars])

def second_assignment(navigations: List[List[str]]) -> int:
...
def generate_correct_answer():
    with open("input.txt", "r") as f:
        ...
        print(first_assignment(navigations))
        print(second_assignment(navigations))
