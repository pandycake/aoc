...
class FishAdministration:
    def __init__(self, fishes: List[int], days: int) -> None:
        # key = (fish, daysleft), value = babies to be had
        self.references = {}
        self.total = 0
    ...
    def _get_pregnancies(self, fish: int, days: int) -> int:
        ...
        if (fish, days) in self.references:  # Check if this pattern has been seen before
            ...
            for baby in range(0, children):
                if baby_days > baby * 7:
                    grandchildren += self._get_pregnancies(
                        6, baby_days - baby * 7 - 2)

        total = children + grandchildren
        # To save calculating in the future
        self.references[(fish, days)] = total
        return total
...
if __name__ == "__main__":
    with open("input.txt", "r") as f:
        ...
        print('Star 1: ' + str(FishAdministration(fishes, 80).total))
        print('Star 2: ' + str(FishAdministration(fishes, 256).total))


# Keeping this to show the story following method of implementation :)
def first_star(fishes: List[int], days: int) -> int:
